...
function preload() {
  ...
  //--------CITATION---------//
  //create background    
  backdrop = loadImage("images/alok-shenoy-100091.jpg"); //--------CITATION---------//     
...
function setup() {
  ...
  jsErrorMessage.style("display:none"); //create the canvas 

  var canvas = createCanvas(windowWidth, windowHeight);
  canvas.position(0, 0);
  canvas.style("z-index", "-1"); //add the music dropzone
  ...
  dropzone.dragLeave(unhighlight); //accept a music file

  dropzone.drop(checkIfMusicReady); //add the background image to the canvas    

  image(backdrop, 0, 0); //add a slider to the interface

  uiSlider = createSlider(12, 60, 48, 12);
  uiSlider.parent("#slider"); //create an instance of amplitude    
...
function draw() {
  if (musicReady) {
    ...
    size = map(level, 0, 1, 2, 50); //first, wait until a flower exists    

    if (garden.length >= 1) {
      launchMessage.remove(); //then plant and grow the flowers
...
},{}]},{},["../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","sketch.js"], null)
